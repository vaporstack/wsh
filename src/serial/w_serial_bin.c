...
//#include <r4/src/io/r_io.h>
#include "../io/w_io.h"

void*		w_document_serialize_bin_v_1(void)
{
	return NULL;
}

void* 		w_document_unserialize_bin_v_1(void)
{
	return NULL;
}
...
WSequence* w_unserialize_sequence_bin(binn* bseq)
{
	...
	binn_object_get_int32 (bseq, "current_frame_index", &seq->current_frame_index);
	binn_object_get_int32 (bseq, "num_golden_frames", &seq->num_golden_frames);
...
WDocument* 	w_document_unserialize_bin(const char* path)
{
	...
	//if ( res != 0 )
	//{
	//	printf("Couldn't read file, file had len 0 or couldn't be opened\n");
	//	return NULL;
	//}
	...
	doc->uuid = binn_object_str(info, "uuid");
	doc->version = binn_object_str(info, "version");
...
binn*			w_serialize_transform_bin(WTransform* trans )
{
...
binn*	w_serialize_object_bin( WObject *obj)
{
	...
	binn_object_set_object(bobj, "transform", w_serialize_transform_bin(&obj->transform));
...
int			w_serialize_sequence_bin( binn* obj, WSequence *seq )
{
	...
	binn_object_set_int32(bseq, "current_frame_index", seq->current_frame_index );
	binn_object_set_int32(bseq, "num_golden_frames", seq->num_golden_frames);
...
int			w_document_serialize_bin(WDocument* doc, const char* path)
{
	...
	strcpy(uuid, doc->uuid);
	binn_object_set_str(info, "version", doc->version );
