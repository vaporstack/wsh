...
void w_line_add_point(WLine* line, WPoint p)
{
	if ( !line )
	{
		printf("Bad codepath, point was added via (drag) but line was never started!\n");
		return;
	...
	// TODO move this somewhere higher level, line is a core data type and shouldn't know about
...
{
	if ( !line ){
		printf("I refuse to destroy something that is already destroyed lol\n");
		return;
	}
	if ( line->data != NULL )
		free(line->data);
