...
void gpc_read_polygon(FILE* fp, int read_hole_flags, gpc_polygon* p)
{
...
#ifdef RPLATFORM_WIN
	fscanf_s(fp, sizeof(fp), "%d", &(p->num_contours));
#else
	...
	for (c = 0; c < p->num_contours; c++) {
...
#ifdef RPLATFORM_WIN
		fscanf_s(fp, sizeof(fp), "%d", &(p->contour[c].num_vertices));
#else
		...
		if (read_hole_flags) {
			...
			fscanf_s(fp, sizeof(fp), "%d", &(p->hole[c]));
#else
		...
		for (v = 0; v < p->contour[c].num_vertices; v++) {
#ifdef RPLATFORM_WIN
			fscanf_s(fp, sizeof(fp), "%lf %lf", &(p->contour[c].vertex[v].x),
...
void gpc_polygon_tr_tristrip(gpc_polygon* s, gpc_tristrip* t)
{
	gpc_polygon c;
